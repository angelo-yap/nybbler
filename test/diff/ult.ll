; Differential test: per-field icmp ult on the SWAR carrier must match
...
define <8 x i2> @ult_i2(<8 x i2> %a, <8 x i2> %b) {
  %c = icmp ult <8 x i2> %a, %b
  %r = sext <8 x i1> %c to <8 x i2>
  ret <8 x i2> %r
}

define <32 x i4> @ult_i4(<32 x i4> %a, <32 x i4> %b) {
